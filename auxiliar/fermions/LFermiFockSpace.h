...
void FCount_rec(int L, int l, Farray vec, int vec_size, int * countAdd, int Nav)
{
...
/** 'L' is the total angular momentum yet to be subtracted.   This function
    ...
    vec = farrDef(lmax); // 'vec' of occupations in l < 0 IPS
...
void FNegMomOcc_rec(int L, int l, Farray vec, int vec_size, int * countAdd,
                    int Navailable, int stride, Farray * ht)
{
...
int FNegMomOcc(int L, int lmax, int Npar, int last_i, Farray * ht)
{
...
/** 'L' is the total angular momentum yet to be subtracted using 'Npar'
    ...
    vec = farrDef(lmax); // 'vec' of occupations in IPS with l < 0
...
Farray * FAssembleHT(int N, int lmax, int totalL, int mcsize)
{
...
/** with the total number of configurations (occupation vectors) given in
    ...
    htable = (Farray *) malloc(mcsize*sizeof(char *));
    for (i = 0; i < mcsize; i++) htable[i] = farrDef(2*lmax+1);

    M = lmax + 1;       // Number of IPS with l >= 0
    occ = farrDef(M);   // occupations in IPS with l >= 0
...
int FgetIndex(int lmax, int ht_size, Farray * ht, Farray occ)
{
